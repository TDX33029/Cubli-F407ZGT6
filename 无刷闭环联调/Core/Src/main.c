...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	delay_ms(1000);            // 等待系统稳定
		...
		controller = Type_velocity;     // 默认启用速度闭环控制模式 (Type_velocity)
		pole_pairs = 7;                 // 极对数

		SimpleFOC_PID_Init();           // 初始化速度闭环 PID 与低通滤波器
		set_motor_enable(0, 1);         // 使能三路 DRV8313 (M1, M2, M3)
		printf("3 motors ready (Closed-loop Speed Mode).\r\n");
	...
	else if (imu_active_type == IMU_TYPE_MPU6050)
	{
		...
		/* 执行三轴无刷电机电角度零位自动对齐与旋向辨识 (使用SysTick计数模式) */
		systick_CountMode();
		Motor_alignAll();
	...
	while(1)
	{
		...
		if(time1_cntr >= 25)
		{
			...
			if (imu_online)
			{
				...
				/* 读取三个电机霍尔/磁编码角度与闭环速度 (MT6701-1, MT6701-2, MT6701-3) */
				hall_online[0] = (i2c_mt6701_1_get_angle(&hall_raw[0], &hall_angle_deg[0]) == 0);
				hall_online[1] = (i2c_mt6701_2_get_angle(&hall_raw[1], &hall_angle_deg[1]) == 0);
				hall_online[2] = (i2c_mt6701_3_get_angle(&hall_raw[2], &hall_angle_deg[2]) == 0);
				...
				{
					/* 扩展标准化遥测格式 (包含三电机目标速度、相电压、使能状态、电角度、六轴陀螺仪加速度、MT6701角度，以及实测滤波转速S1/S2/S3) */
			...
			else if(strncmp(cmd, "SENSOR", 6) == 0 || strncmp(cmd, "sensor", 6) == 0)
			{
				        ...
				        strncmp(cmd, "ENC", 3) == 0 || strncmp(cmd, "enc", 3) == 0)
				{
					printf("--- Motor Hall/Encoder Sensors (MT6701) ---\r\n"
					       ...
					       hall_raw[1], (uint16_t)hall_raw[1], hall_angle_deg[1], shaft_velocity[1], hall_online[1],
					       hall_raw[2], (uint16_t)hall_raw[2], hall_angle_deg[2], shaft_velocity[2], hall_online[2]);
				}
				/* 12. 控制模式切换: MODE <0/1> (0=开环 Type_velocity_openloop, 1=闭环 Type_velocity) */
				...
				/* 14. PID 参数设定: PID <p> <i> <d> 或 PID1/2/3 <p> <i> <d> */
