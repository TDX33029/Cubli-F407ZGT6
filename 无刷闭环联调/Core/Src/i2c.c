...
/* USER CODE BEGIN 0 */
#define MT6701_READ_TIMEOUT 2  // 单次读超时优化为 2ms，避免总线离线阻塞 CPU 主循环
...
unsigned char mt6701_write_reg(I2C_HandleTypeDef *hd_i2c, unsigned char reg, unsigned char value)
{
	...
	if (res != HAL_OK) hd_i2c->State = HAL_I2C_STATE_READY;
	return res;
}
...
{
    uint8_t temp[2] = {0};
    if (HAL_I2C_Mem_Read(&hi2c1, MT6701_SLAVE_ADDR, MT6701_REG_ANGLE_14b, I2C_MEMADD_SIZE_8BIT, temp, 2, MT6701_READ_TIMEOUT) == HAL_OK)
    {
